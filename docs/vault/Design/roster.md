...
2. **Cleric** (support, ranged r3) — innate: small heal trickle to lowest ally /
   ...
   *Lifebinder*: Mend splashes to allies adjacent to the target.
...
5. **Pyromancer** (caster, ranged r3, weak auto / big mana) — innate: attacks apply 1 Burn /
   sig **Fireball**: Splash (target + ring), applies Burn →
   *Inferno*: Burning enemies that die spread Burn | *Starfall*: no Splash, triple damage single target.
...
7. **Phalanx** (reach fighter, melee r2 — hits over the front row) — innate: Counters the
   ...
   *Pikewall*: Counter always, +armor stance | *Lancer*: Skewer becomes a full-line lunge.
...
❓ Open reactions wanted: which chassis feel wrong/missing? (No dedicated summoner,
displacer, or debuffer in v1 — deliberate scope cuts, all good post-v1 candidates.)
